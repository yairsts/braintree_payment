...
class VenmoActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val token: String = intent.getStringExtra(VenmoConstants.TOKEN_KEY) as String
        val displayName: String = intent.getStringExtra(VenmoConstants.DISPLAY_NAME_KEY) as String
        val amount: String = intent.getStringExtra(VenmoConstants.AMOUNT_KEY) as String
        val appLinkReturnUrl: String =
            intent.getStringExtra(VenmoConstants.APP_LINK_RETURN_URL) as String
        val deepLinkFallbackUrlScheme: String? =
            intent.getStringExtra(VenmoConstants.DEEP_LINK_FALLBACK_URL_SCHEME)

        venmoLauncher = VenmoLauncher()
        venmoClient = VenmoClient(
            context = this,
            authorization = token,
            appLinkReturnUrl = Uri.parse("$appLinkReturnUrl.braintree"),
            deepLinkFallbackUrlScheme = "$deepLinkFallbackUrlScheme.braintree",
        )
    ...
    private fun handleSuccessResult(result: VenmoResult.Success) {
        ...
        val resultIntent = Intent().apply {
            putExtra(VenmoConstants.VENMO_NONCE_KEY, nonceJson.toString())
        }
    ...
    private fun handleErrorResult(error: Exception) {
        ...
        val resultIntent = Intent().apply {
            putExtra(VenmoConstants.VENMO_ERROR_KEY, error.toString())
        }
    ...
    private fun handleCancelResult(message: String) {
        ...
        val resultIntent = Intent().apply {
            putExtra(VenmoConstants.VENMO_CANCELED_KEY, message.toString())
        }
        setResult(Activity.RESULT_CANCELED, resultIntent)
        finish()
    }
